...
RSpec.describe ClassApiController, type: :controller do
  ...
  it 'allows the user to retrieve the fuzzy search count of a resource' do
    post :count, params: { model: 'customer', search: { name: 'a' } }

    expect(JSON.parse(response.body)['customers_count']).to eq Customer.where("name like '\%a\%'").count
  end
  ...
  it 'allows a user to perform a fuzzy search' do
    post :index, params: { model: 'customer', search: { name: 'a' } }

    expect(response.code).to eq('200')
  end

  it 'allows a user to perform a scoped fuzzy search' do
    post :index, params: { model: 'customer', customer: { stuff: 'bob' }, search: { name: 'a' } }

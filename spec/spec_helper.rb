...
RSpec.configure do |config|
  ...
  SimpleCov.start :rails do
    add_filter do |src|
      src.filename =~ /^#{SimpleCov.root}/ if src.filename =~ /dummy/
    end

    add_filter do |src|
      src.filename =~ /^#{SimpleCov.root}/ if src.filename =~ /generator/
    end

    add_filter do |src|
      src.filename =~ /^#{SimpleCov.root}/ if src.filename =~ /identity/
    end
  end
end
